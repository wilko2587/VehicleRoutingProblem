...
class VehicleRouterEnvironment:
    ...
    def _setup(self, problem_file_index=0, scaler=None):
        ...
        self.max_time = self.data["DUE_DATE"].max()
    ...
    def state(self):
        ...
        state.drop("DUE_DATE", axis=1, inplace=True)
        state.drop("READY_TIME", axis=1, inplace=True)
        ...
        vehicle_load = [v.load for v in self.vehicles]
        my_load = self.vehicle.load
        return np.array(list(state.values.flatten()) + [0 if i not in vehicle_pos else 1 for i in range(len(self.data))] + vehicle_load + [my_load])
    ...
    def get_stats(self):
        ...
        '''
        return self.time, self.demand_completed, round(self.data.loc[:, "DEMAND"].sum(), 1)

    def step(self, action):
        # returns next_state, reward, done, _

        demand = self.data.loc[action, "DEMAND"]
        if self.vehicle.load + demand > self.load_limit:
            action = 0 # send to depot
        ...
        if demand > 0 and action != 0:
            next_time = prior_time + move_time + self.service_time
            reward = 0
            self.vehicle.load += demand
            self.data.loc[action, "DEMAND"] = 0
        elif demand == 0 and action != 0: # if seeing someone who's been serviced before
            next_time = prior_time + move_time
            reward = 0
        elif action == 0: # if at depot
            self.demand_completed += self.vehicle.load
            reward = self.vehicle.load
            self.vehicle.load = 0 # unload
            next_time = prior_time + move_time
        else:
            raise ArithmeticError("Is demand below zero??? {}".format(demand))
        ...
        done = (self.data.loc[:, 'DEMAND'].sum() == 0) and (sum([v.load for v in self.vehicles]) == 0.)
